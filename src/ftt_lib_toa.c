...
T(ulto_bl_easy_0)
{
	...
	nb = 0;
	target = ftt_malloc(2 * sizeof(char));
	result_size = ft_ultoa_bl(nb, target, 1, "01");
...
T(ltoa_b_neg_min)
{
	...
	expected = ft_str_alloc(20);
	sprintf(expected, "%ld", LONG_MIN);
	actual = ft_ltoa_b(LONG_MIN, FT_ALPHABET_DEC);
...
T(ltoa_b_pos_max)
{
	...
	actual = ft_ultoa_b(LONG_MAX, FT_ALPHABET_DEC);
	cr_assert_str_eq(expected, actual);
	free(actual);
	free(expected);
}
...
T(itoa_simple)
{
	...
	cr_assert_str_eq(s = ft_itoa(42), "42");
	free(s);
	cr_assert_str_eq(s = ft_itoa(-1), "-1");
	free(s);
	cr_assert_str_eq(s = ft_itoa(0), "0");
	free(s);
}
...
T(itoa_binary_base)
{
	...
	cr_assert_str_eq(s = ft_itoa_b(1, "0X"), "X");
	free(s);
	cr_assert_str_eq(s = ft_itoa_b(0, "0X"), "0");
	free(s);
}
...
T(itoa_binary_base_larg)
{
	...
	cr_assert_str_eq(s = ft_itoa_b(2, "01"), "10");
	free(s);
	cr_assert_str_eq(s = ft_itoa_b(42, "01"), "101010");
	free(s);
	cr_assert_str_eq(s = ft_itoa_b(4, "0X"), "X00");
	free(s);
}
...
T(itoa_INT_MAX)
{
	...
	s_int_max = (char *)malloc(15 * sizeof(char));
